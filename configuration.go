...
func GetSignRequest(requestParam RequestParam, credentials Credential) SignRequest {
	...
	if requestParam.IsSignUrl {
		...
		requestSignMap["X-Date"], requestSignMap["Host"] = []string{formatDate}, []string{requestParam.Host}
...
func getMetaData(credentials Credential, date string) *metadata {
	meta := new(metadata)
...
func buildAuthHeaderV4(signature string, meta *metadata, keys Credential) string {
